...
/** Catalog card. Clicking it opens the product modal; the + button quick-adds
 *  to the bag without opening detail (design_ref behaviour). */
export function StoreProductCard({ product, onOpen, onAdd }: StoreProductCardProps) {
  ...
  return (
    <article
      ...
      className="group cursor-pointer outline-none transition-transform duration-200 hover:-translate-y-1"
    >
      <ProductImage
      ...
      >
        {product.tag && <Tag className="absolute left-2 top-2 z-[1]">{product.tag}</Tag>}
        <button
          type="button"
